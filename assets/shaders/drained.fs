...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    //vec2 rotater = vec2(cos(drained.r*0.1221), sin(drained.r*0.3512));


    if (drained.g > 0.0 || drained.g < 0.0) {
        hsl.y = 0.02;
        hsl.z *= (1 - adjusted_uv.x*(cos(drained.r*0.512)));
        hsl.z *= (1 - adjusted_uv.y*(cos(drained.r*0.512)));
    }
